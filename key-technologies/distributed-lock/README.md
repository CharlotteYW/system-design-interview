...
| --- | --- |
| DB row lock / transaction | TBD — short, same database |
| Redis SET NX + TTL | TBD — simple distributed lock |
| Redlock / ZooKeeper | TBD — multi-instance; extra complexity |
| Fencing tokens | TBD — expire then old holder still writes |
| Granularity | TBD — one seat vs a section |
| Deadlock | TBD — lock order; never lock A then B vs B then A |
...
Redis SET key NX EX: two workers, only one holds the ticket lock. Show expiry after crash.
...
```bash
cd key-technologies/distributed-lock
./scripts/setup.sh
...
None yet. Prompt `Distributed Lock` to fill this folder.
